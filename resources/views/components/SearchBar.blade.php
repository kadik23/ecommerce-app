...
@endif
    @csrf       
    <div class="flex ">
        <label for="search-dropdown" class="mb-2 text-sm font-medium text-gray-900 sr-only dark:text-white">{{ t('admin.products.search') }}</label>
        ...
        <select name="category"  id="category" class="flex-shrink-0 active:border-none focus:border-none ml-3 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-gray-900 bg-gray-100 border border-gray-300 rounded-l-lg hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-700 dark:text-white dark:border-gray-600">
            <option value="All categories" {{ request('category') == 'All categories' || !request('category') ? 'selected' : '' }}>{{ t('admin.products.all_categories') }}</option>
            @foreach($categories AS $category)
            <option value="{{$category->name}}" {{ request('category') == $category->name ? 'selected' : '' }} class="opt p-16">
                {{$category->name}}
            </option>
            @endforeach
        </select>
        ...
        <div class="relative w-96 hover:opacity-90 transition-opacity duration-500">
            <input type="search" name="search" id="search-dropdown" value="{{ request('search') }}" class="block  p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-r-lg border-l-gray-50 border-l-2 border border-gray-300 focus:ring-regal-brown focus:border-regal-brown dark:bg-gray-700 dark:border-l-gray-700  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-regal-brown" placeholder="{{ t('admin.products.search_placeholder') }}" >
            <button type="submit"  class="absolute top-0 right-0 p-2.5 text-sm font-medium h-full text-white bg-regal-brown rounded-r-lg border border-regal-brown hover:bg-regal-brown focus:ring-4 focus:outline-none focus:ring-amber-600 dark:bg-regal-brown dark:hover:bg-regal-brown dark:focus:ring-regal-brown">
                ...
                <span class="sr-only">{{ t('admin.products.search') }}</span>
            </button>
        </div>
    </div>
</form>